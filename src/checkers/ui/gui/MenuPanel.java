...
public class MenuPanel extends JPanel {
    ...
    private final String loggedInUser; // null when guest

    // Color scheme - minimalistic and clean (unchanged)
    ...
    public MenuPanel(CheckersGUI parent, String loggedInUser) {
        this.parent       = parent;
    ...
    private void initComponents() {
        ...
        // Subtitle - show account status
        String subText = (loggedInUser != null)
                ? "Logged in as " + loggedInUser
                : "6x6 Board Game";
        JLabel subtitleLabel = new JLabel(subText);
        ...
        player1Field = createStyledTextField("Enter Player 1 name");
        if (loggedInUser != null) {
            player1Field.setText(loggedInUser);
            player1Field.setForeground(ACCENT_COLOR);
        }
        ...
        player2Label.setForeground(new Color(50, 50, 50));
        player2Label.setBounds(150, 380, 300, 25);
        add(player2Label);

        player2Field = createStyledTextField("Enter Player 2 name");
        ...
        player1Field.addKeyListener(new KeyAdapter() {
            @Override public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_ENTER) player2Field.requestFocus();
            }
        });
        ...
        startButton.setBounds(150, 490, 300, 50);
        startButton.addActionListener(e -> startGame());
        add(startButton);

        // Stats / Logout row (only for logged-in users)
        if (loggedInUser != null) {
            JButton statsButton = createStyledButton("MY STATS");
            statsButton.setBounds(150, 550, 140, 36);
            statsButton.addActionListener(e -> parent.showStats());
            add(statsButton);

            JButton logoutButton = createStyledButton("LOG OUT");
            logoutButton.setBounds(310, 550, 140, 36);
        ...
        JLabel instructLabel = new JLabel(
                "<html><center>Click on a piece to select it,<br/>"
                + "then click on a valid square to move</center></html>");
        instructLabel.setFont(new Font("Arial", Font.PLAIN, 12));
        instructLabel.setForeground(new Color(150, 150, 150));
        instructLabel.setBounds(0, 610, 600, 50);
    ...
    private void startGame() {
        ...
        if (p1.isEmpty() || p1.equals("Enter Player 1 name")) p1 = (loggedInUser != null) ? loggedInUser : "Player 1";
        if (p2.isEmpty() || p2.equals("Enter Player 2 name")) p2 = "Player 2";
